...
- Keep `crates/tauri-dev-core` free of CLI output and process side effects. It should expose config, state, diagnostics, and plan primitives.
- Keep `crates/tauri-dev-cli` as the installed binary boundary named `tauri-dev`.
...
- CLI smoke: `cargo run --locked -p tauri-dev-cli -- doctor --config examples/minimal.toml`

## Repository Shape

- `crates/tauri-dev-core/`: reusable model, config loading, diagnostics, and execution planning.
- `crates/tauri-dev-cli/`: CLI parsing, text/JSON output, and process exit behavior.
