...
USER_AGENT = "sidecar-release-stable/1.0"
...
def _try_fetch(url: str) -> tuple[str | None, int | None]:
    ...
    if result.returncode != 0:
        fail(f"failed to fetch R2 stable metadata: {result.stderr.strip() or result.returncode}")
        return None, None

    body, separator, status_text = result.stdout.rpartition("\n")
    if not separator:
        fail("failed to fetch R2 stable metadata: missing HTTP status")
    try:
        status = int(status_text)
    except ValueError:
        fail(f"failed to fetch R2 stable metadata: invalid HTTP status {status_text!r}")
    if 200 <= status < 300:
        return body, None
    return None, status
...
def next_stable(cargo_version: str) -> tuple[str, str, str]:
    ...
    metadata_url = os.environ.get("SIDECAR_STABLE_METADATA_URL")
    if not metadata_url:
        if not public_url:
    ...
    if ordering == 0:
        fail(
            ...
            f"crates/cli/Cargo.toml before re-running"
        )
